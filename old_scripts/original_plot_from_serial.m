...
while(true)
    ...
    longdata = longdata((end+1-(256*17)):end);
    s = length(longdata)
    ...
    for n = 1:17
            subplot(5,4,n)
    hold on;    
    plot(1:s(2),parseddata(mod(a+n-2,17)+1,:),'k-','Color',d_getgoodcolour(colourindexlist(n)))

    axis([0 s(2) 0 255]);
    end
    xlabel('Sample', 'FontSize', 12);
    ylabel('Value', 'FontSize', 12);
    set(gcf,'Position',[95 566 560 420]);
    ...
    for n = 1:6
        subplot(2,3,n)
    plot(1:s(2),trace(n,:),'k-','LineWidth', 1,'Color', d_getgoodcolour(n));
    axis([0 s(2) 0 1]);
    if n==4
        xlabel('Sample', 'FontSize', 12);
    ylabel('Value', 'FontSize', 12);
    ...
    set(gcf,'Position',[654 566 560 420]);
    ...
    % figure(3);
    % hold on;
    % plot(1:s(2),channels(1,:),'k-','LineWidth',1,'Color', d_getgoodcolour(1));
    % plot(1:s(2),channels(2,:),'k-','LineWidth',1,'Color', d_getgoodcolour(3));
    % plot(1:s(2),channels(3,:),'k-','LineWidth',1,'Color', d_getgoodcolour(5));
    % plot(1:s(2),channels(4,:),'k-','LineWidth',1,'Color', d_getgoodcolour(6));

    axis([0 s(2) 0 1]);
    set(gcf,'Position',[1204 566 560 420]);
    ...
    % plot(xvals,ch1ifft,'k-','LineWidth',2,'Color', d_getgoodcolour(1));
      plot(xvals,ch2ifft,'k-','LineWidth',2,'Color', d_getgoodcolour(3));
    %  plot(xvals,ch3ifft,'k-','LineWidth',2,'Color', d_getgoodcolour(5));
    %  plot(xvals,ch4ifft,'k-','LineWidth',2,'Color', d_getgoodcolour(6));
